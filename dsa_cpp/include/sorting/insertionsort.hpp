...
void
insertion_sort(RandomAccessIter first, RandomAccessIter last, Comparator comp) {
    if (first == last) {
        return;
    }
    if (last == first + 1) {
    ...
    // A first pass through the array to place the smallest/largest key in the first
    // position depending upon the `comp` function. 
...
void
insertion_sort(RandomAccessIter first, RandomAccessIter last) {
    using value_type =
